...
const countriesReducer = function(state = initialState, action) {
    switch (action.type) {
        ...
        case GET_COUNTRY:
            const selectedCountry = state.countries.find(country => country.alpha2Code == action.alpha2Code);
        ...
        case DELETE_COUNTRY:
            const notDeletedCountries = state.countries.filter(country => country.alpha2Code != action.alpha2Code);
            const notDeletedVisibleCountries = state.visibleCountries.filter(country => country.alpha2Code != action.alpha2Code);
